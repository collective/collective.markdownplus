...
- Basic markdown coloring in preview output (headings, bold, italic, code)
- Toolbar: Bold, Italic, Code block, Link insertion buttons
...
1. Runs ``plonex init .`` from the package root.
   ``plonex`` finds ``etc/plonex.yml`` here, creates ``.venv``, and installs
   Plone + this package (``-e .`` via ``etc/requirements.d/``).
2. Creates an admin user via ``plonex adduser``.
3. Runs ``scripts/bootstrap_site.py`` (via ``plonex run``) to create the Plone
   site and apply the ``collective.markdownplus:default`` GenericSetup profile.
...
.. code-block:: bash
   ...
   plonex zeoclient fg   # or just: plonex
...
The widget will render a split-pane editor (left) and live preview (right).
Standard ``text/html`` fields continue to use TinyMCE as usual.
